...
SSH.prototype._queueCommand = function (index) {
    ...
    this._c.exec(cmd, { pty: command.pty }, function (err, stream) {
        ...
        stream.on('close', function (code) {
            const numOfCommands = self._commands.length;
            const executeNext = (command.handlers.exit(+code, stdout, stderr) !== false);

            if (!executeNext && numOfCommands !== self._commands.length) {
